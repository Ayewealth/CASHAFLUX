...
import {
  ArrowUpRight, CheckCircle2, ChevronDown, Receipt, Banknote, BarChart3,
  PieChart, FileText, Users, Quote, Sparkles,
} from 'lucide-react'
...
const HOW_IT_WORKS = [
  { step: 1, title: 'Connect your business', desc: 'Set up your profile, add your clients, and link your bank accounts — takes less than 5 minutes.' },
  { step: 2, title: 'Manage your finances', desc: 'Create invoices, log expenses, reconcile transactions, and track mileage — all from one dashboard.' },
  { step: 3, title: 'Stay tax-ready', desc: 'Generate reports, track quarterly deadlines, and export tax-ready packages for your accountant.' },
]

const TESTIMONIALS = [
  { name: 'Sarah Chen', role: 'Freelance Designer', company: 'Chen Creative', quote: 'Cashaflux saved me hours every month. Invoicing used to be a chore, now it takes me 30 seconds.' },
  { name: 'Marcus Johnson', role: 'Owner', company: 'Johnson & Co. Landscaping', quote: 'The bank reconciliation feature alone is worth it. Matches everything automatically — no more manual spreadsheet work.' },
  { name: 'Emily Rodriguez', role: 'CPA', company: 'Rodriguez Tax Services', quote: 'I recommend Cashaflux to all my small business clients. The tax-ready export package makes my job so much easier.' },
  { name: 'David Kim', role: 'Sole Trader', company: 'Kim Consulting', quote: "I was using spreadsheets for years. Cashaflux made the switch painless. I wish I'd done it sooner." },
]
...
const PLANS = [
  {
    id: 'free', name: 'Free', price: 0, features: ['Up to 5 clients', 'Core invoicing', 'Expense tracking', 'Basic reports', 'Mileage tracking'],
    popular: false, cta: 'Start Free',
  },
  {
    id: 'pro', name: 'Pro', monthly: 19, annual: 180,
    features: ['Unlimited clients', 'Bank sync & reconciliation', 'Recurring invoices', 'Advanced reports', 'Tax-ready exports', 'Priority email support'],
    popular: true, cta: 'Choose Pro',
  },
  {
    ...
    popular: false, cta: 'Choose Business',
  },
]
...
const COMPARISON_ROWS = [
  ...
  { feature: 'Tax-ready exports', values: [{ label: 'QuickBooks', included: true }, { label: 'Xero', included: false }, { label: 'Spreadsheets', included: false }, { label: 'Cashaflux', included: true }] },
  { feature: 'Mileage tracking', values: [{ label: 'QuickBooks', included: true }, { label: 'Xero', included: false }, { label: 'Spreadsheets', included: 'Manual' }, { label: 'Cashaflux', included: true }] },
  { feature: 'Modern UI', values: [{ label: 'QuickBooks', included: false }, { label: 'Xero', included: true }, { label: 'Spreadsheets', included: false }, { label: 'Cashaflux', included: true }] },
...
export default function HomePage() {
  usePageMeta({ title: 'Accounting for Small Businesses', description: 'Simple, fast accounting for American small businesses. Invoicing, expense tracking, bank reconciliation, and tax-ready reports.', ogType: 'website', canonical: 'https://cashaflux.com/' })
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            aggregateRating: { '@type': 'AggregateRating', ratingValue: 4.8, ratingCount: 256 },
          }),
        }}
      />
      <Header />
      ...
      <section className="min-h-[100dvh] pt-20 lg:pt-0 flex items-center relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8 w-full">
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center py-16 lg:py-24">
            <div>
              <SmoothScrollReveal delay={0}>
                <span className="inline-flex items-center gap-1.5 px-3 py-1 text-xs font-medium text-brand-navy bg-brand-blue-light rounded-full mb-6">
                  <Sparkles className="w-3.5 h-3.5" />
                  Built for American small businesses
                </span>
              </SmoothScrollReveal>
              <SmoothScrollReveal delay={0.1}>
                <h1 className="text-4xl sm:text-5xl lg:text-6xl xl:text-7xl font-bold text-brand-navy tracking-tighter leading-[1.08] mb-6">
                  Simple accounting.{' '}
                  <span className="text-brand-navy-light">Built for America.</span>
                </h1>
              </SmoothScrollReveal>
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <SmoothScrollReveal>
            <div className="text-center max-w-2xl mx-auto mb-16">
              ...
              <p className="text-base text-text-muted leading-relaxed">No complicated setup. No training required.</p>
            </div>
          </SmoothScrollReveal>
              ...
              <SmoothScrollReveal key={step.step} delay={0.15 * i}>
                <div className="flex gap-6 lg:gap-10">
                  ...
                  <div className="pt-2">
                    ...
                    <p className="text-sm text-text-muted leading-relaxed max-w-md">{step.desc}</p>
                  </div>
                </div>
              </SmoothScrollReveal>
            ))}
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          ...
          {reduce ? (
            <div className="grid md:grid-cols-2 gap-6 max-w-4xl mx-auto">
              {TESTIMONIALS.map((t) => (
                <div key={t.name} className="p-6 rounded-2xl bg-white border border-border/50 hover:shadow-md hover:shadow-brand-navy/5 transition-all duration-300">
              ...
              <SmoothScrollReveal key={integration.name} delay={0.08 * i}>
                <div className="p-4 rounded-2xl bg-white border border-border/50 text-center hover:border-brand-navy/20 hover:shadow-md transition-all duration-300">
                  <div className="w-10 h-10 rounded-xl bg-surface flex items-center justify-center mx-auto mb-3 border border-border/50">
                    <span className="text-lg font-bold text-brand-navy font-mono">{integration.name[0]}</span>
                  </div>
                  <h3 className="text-xs font-bold text-brand-navy mb-1">{integration.name}</h3>
                  <p className="text-[10px] text-text-muted leading-relaxed">{integration.desc}</p>
                </div>
              </SmoothScrollReveal>
            ))}
          </div>
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <SmoothScrollReveal>
            <div className="text-center max-w-2xl mx-auto mb-16">
              ...
              <p className="text-base text-text-muted leading-relaxed mb-6">Start free. Upgrade when you grow.</p>
              <div className="flex justify-center">
                <div className="flex gap-1 bg-white p-0.5 rounded-lg border border-border/50">
                  ...
                  >
                    Annual <span className="text-[10px] text-brand-navy-light ml-0.5">Save 20%</span>
                  </button>
                </div>
              </div>
            </div>
          ...
          <div className="grid md:grid-cols-3 gap-6 max-w-5xl mx-auto">
            {PLANS.map((plan, i) => {
              const price = 'monthly' in plan
                ? (billingInterval === 'annual' ? `$${plan.annual}` : `$${plan.monthly}`)
                : '$0'
              const period = 'monthly' in plan
